...
export default async function StatsPage({ params }: PageProps) {
  ...
  const linkData = await getLinkStats(code);
  ...
  const shortUrl = `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/${linkData.code}`;
        ...
        <div className="bg-white rounded-lg shadow-md p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="flex flex-col">
              <div className="flex items-center justify-between mb-2">
                ...
                <CopyButton text={linkData.code} />
              </div>
              <code className="text-2xl font-bold text-teal-600">{linkData.code}</code>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-6 border-t">
            ...
            <div className="bg-linear-to-br from-teal-50 to-teal-100 rounded-lg p-6 border border-teal-200">
              ...
              <p className="text-4xl font-bold text-teal-900 mt-3">{linkData.clicks}</p>
            </div>
              ...
              {linkData.last_clicked_at ? (
                <>
                  ...
                  <p className="text-xs text-purple-600 mt-1">
                    {new Date(linkData.last_clicked_at).toLocaleTimeString()}
                  </p>
                </>
              ) : (
